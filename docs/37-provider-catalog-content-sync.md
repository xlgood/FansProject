...
- 客户可见页面、接口和文案不得出现或暗示 FansGurus、TGX、上游、供应商、货源、外部商家、外部客服或外部链接。
...
2. 删除 URL、邮箱以及带 URL/邮箱的整行，避免留下无意义片段或引流信息。
3. 丢弃含外部来源或联络语义的整行，例如品牌名、`upstream`、`supplier`、`provider`、`merchant`、上游、供应商、货源、服务商、平台商家、来源、联系客服、WhatsApp、Telegram、`t.me`。
4. 保留可验证且与下单有关的内容，例如数量范围、质量、开始时间、速度、账号格式、使用注意事项和目标链接要求。
5. FansGurus 服务类型为 `Custom Comments`（或等价的中英文名称）时，额外生成本站填写说明：目标链接必填、每行一条评论、数量与金额按有效评论行数自动计算。
...
3. 抽查 TGX 商品：客户页显示清洗后的 API 描述，不显示外部网址、邮箱、品牌、上游、供应商、商家或客服联络方式。
