...
2026-07-21 的首次资料同步成功更新 2,932 个映射商品后，现场复核发现两类补充规则：TGX 新商品在未执行真实库存刷新前显示“库存待确认”并禁止购买，这是目录接口不提供可信库存时的预期安全状态，应通过后台单个“刷新库存”或受控全量 TGX 库存同步解决；不能由资料同步伪造库存。另一方面，客户描述不能只删除外部工具品牌、下载和导入步骤后留下空白：必须把可独立使用的交付字段与使用方式改写为本站说明，例如保留邮箱地址、密码、客户端标识、授权信息以及 OAuth2/POP/IMAP 配置要求；明确标记为客户登录/使用教程的链接应保留为安全的“查看教程”链接。详细清洗准则已更新至 `docs/37-provider-catalog-content-sync.md`。

2026-07-21 进一步确认：TGX 的 `widget`、`contact_type`、`delivery_way` 不能按来源统一解释，必须按每条 `upstream_product_code` 独立读取和保存。资料同步现对每个已启用 TGX 映射在共享限流下调用单商品资料接口，并将该商品自己的购买字段与自动/人工交付展示写入本地；单商品失败时保留目录回退资料，绝不猜测字段类型或复制另一个 SKU 的规则。映射层支持人工核验后的锁定覆盖，避免后续同步覆盖已确认的单 SKU 例外。已核验的 `A821AB6AEEECA7C1` 仅被锁定为 `contact` 键的邮箱字段，不能推广到其他商品。客户表单新增字段说明显示；订单校验和提交仍使用原始字段键。

同时新增本站原创的 `/guides/account-access`，仅从本地邮箱账号分类的商品详情进入。指南不含 Telegram，不链接、复制或提及第三方页面、品牌和外部处理方；只说明在本站订单中核对交付字段、使用官方登录入口以及在订单明确提供资料时配置 OAuth2/POP/IMAP。该项仅改善使用说明，不改变订单、库存、价格或上架状态。
