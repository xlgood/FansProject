...
2026-07-21 的首次资料同步成功更新 2,932 个映射商品后，现场复核发现两类补充规则：TGX 新商品在未执行真实库存刷新前显示“库存待确认”并禁止购买，这是目录接口不提供可信库存时的预期安全状态，应通过后台单个“刷新库存”或受控全量 TGX 库存同步解决；不能由资料同步伪造库存。另一方面，客户描述必须删除外部工具品牌以及依赖其下载、导入、解析的后续步骤，不能留下无法执行的残余文本；但明确标记为客户登录/使用教程的链接应保留为安全的“查看教程”链接。详细清洗准则已更新至 `docs/37-provider-catalog-content-sync.md`。
